# Annex C — Trades Matrix

**Document ID:** RADR / ANX-C  
**Version:** 1.2.0  
**Status:** Conceptual

---

## Selected

| Trade | Rejected | **RADR** |
|-------|----------|----------|
| Role | Divisional SAM | **Squad/SOF mid-range** |
| Threats | Armor, aircraft | **Group 1–2 UAS (5 classes)** |
| Payload | Rod, HE only | **300×7 mm alloy cone** |
| Guidance | Beam-ride, launcher track, high OBA | **IR F&F + moderate-maneuver nose canards** |
| Fuze | Impact / command | **Radar or mm-wave proximity + timed backup** |
| Motor | Boost-first | **Mildly progressive 750–850 N → 1050–1150 N peak** |

---

## Excluded Concepts

Laser beam-riding · Launcher-tracked guidance · Kinetic penetrator rod · High off-boresight · FMCW radar seeker

---

[← 05 — Key Design Trades](../docs/05-key-design-trades.md)
